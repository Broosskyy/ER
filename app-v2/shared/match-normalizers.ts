...
function singleTokenSubstitutionSimilarity(a: string, b: string): number | null {
  const aTokens = a.split(' ').filter(Boolean);
  const bTokens = b.split(' ').filter(Boolean);
  if (aTokens.length !== bTokens.length || aTokens.length === 0) {
    return null;
  }
  let diffs = 0;
...
export function titleSimilarity(left: string, right: string): number {
  ...
  const substitutionScore = singleTokenSubstitutionSimilarity(a, b);
  if (substitutionScore != null) {
...
export function canonicalTicketUrlForCompare(url: string | null | undefined): string | null {
  if (!url?.trim()) {
    return null;
  }
  try {
