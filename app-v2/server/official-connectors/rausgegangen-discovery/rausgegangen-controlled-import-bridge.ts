...
import { dedupeDescription, qualifyLineup } from '../ticket-evidence/network-discovery/field-evidence';
...
import { RAUSGEGANGEN_DISCOVERY_CONNECTOR_ID } from './constants';

export { RAUSGEGANGEN_DISCOVERY_CONNECTOR_ID };

export function buildRausgegangenPageFingerprint(
    ...
    return createHash('sha256')
      .update(
        JSON.stringify({
          ...
          ticketUrl: enriched.outboundLinks[0] ?? enriched.eventUrl,
        }),
      )
      .digest('hex');
  }
...
function buildLineupCandidates(lineup: string[]): OfficialLineupCandidate[] {
  const filtered = lineup
    ...
    .filter(
      (name) =>
        name.length > 0 &&
        !/^(?:early bird|sold out|tickets?|phase \d+|tba|tbc|tbd|coming soon|folgt|weitere folgen)$/i.test(name),
    );
...
export function buildOfficialEvidenceFromRausgegangenEnriched(
  enriched: EnrichedTicketIoEvent,
  fetchResult: DetailFetchResult,
  fetchedAt: string,
  ...
  const ticketUrl = enriched.outboundLinks[0] ?? enriched.eventUrl;
  ...
  return {
    connectorId: RAUSGEGANGEN_DISCOVERY_CONNECTOR_ID,
    sourceEventKey: enriched.identityKey,
    listUrl: enriched.listingUrl,
    officialUrl: enriched.canonicalUrl || enriched.eventUrl,
    fetchedAt,
    pageFingerprint: buildRausgegangenPageFingerprint(fetchResult, enriched),
    title: enriched.title,
    startsAt: enriched.startsAt!,
...
export interface FinalizeRausgegangenImportResult {
  evidence: OfficialEventEvidence;
  ticketResult?: VerifiedTicketCompleteResult;
}

export async function finalizeRausgegangenControlledImportEvidence(
  enriched: EnrichedTicketIoEvent,
  fetchResult: DetailFetchResult,
  fetchedAt: string,
): Promise<FinalizeRausgegangenImportResult> {
  const baseEvidence = buildOfficialEvidenceFromRausgegangenEnriched(enriched, fetchResult, fetchedAt);
...
export interface RausgegangenLiveVerification {
  identityKey: string;
  sourceUrl: string;
  verifiedAt: string;
...
export function isEligibleForRausgegangenControlledImport(
  ...
  if (verification.detailAccess !== 'DETAIL_ACCESSIBLE' && verification.detailAccess !== 'PARTIAL_DETAIL') {
  ...
  if (verification.startsAt) {
    ...
    if (startsAt < referenceInstant && verification.ticketAvailability === 'SOLD_OUT') {
...
export function reviewRausgegangenWritePlan(plan: EventWritePlan): Array<{
  identityKey: string;
  severity: 'block' | 'warn';
  code: string;
  detail: string;
}> {
  const issues: Array<{ identityKey: string; severity: 'block' | 'warn'; code: string; detail: string }> = [];
  ...
  if (imageUrl && /rausgegangen\.de\/static|logo|placeholder/i.test(imageUrl) && !/flyer|event|s3\.eu-central-1/i.test(imageUrl)) {
    issues.push({
      identityKey,
      severity: 'block',
...
export function summarizeRausgegangenWritePlanMutations(plans: EventWritePlan[]) {
  return plans.reduce(
    (acc, plan) => {
      ...
      acc.descriptionWrites += plan.eventAction === 'update' ? 1 : 0;
      acc.mediaWrites += plan.eventAction === 'update' && plan.candidate.imageUrl ? 1 : 0;
