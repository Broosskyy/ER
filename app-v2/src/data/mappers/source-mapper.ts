...
export function mapSourceRecordToRow(record: SourceRecord): SourceRow {
  ...
  return {
    ...
    consecutive_failure_count: record.consecutiveFailureCount ?? 0,
    total_import_count: record.totalImportCount ?? 0,
    total_valid_event_count: record.totalValidEventCount ?? 0,
    total_rejected_event_count: record.totalRejectedEventCount ?? 0,
    duplicate_rate: record.duplicateRate ?? 0,
    update_rate: record.updateRate ?? 0,
    error_rate: record.errorRate ?? 0,
    ...
    schedule_enabled: record.scheduleEnabled ?? false,
    schedule_policy: record.schedulePolicy ?? 'manual_only',
    schedule_timezone: record.scheduleTimezone ?? 'UTC',
    schedule_interval_preset: record.scheduleIntervalPreset ?? 'manual',
    scheduler_maintenance_mode: record.schedulerMaintenanceMode ?? false,
